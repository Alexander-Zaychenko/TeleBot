...
import telebot
from games.rock_scissors_paper import in_game, engine
...
#         bot.send_message(message.chat.id, "Ничья! (выход - exit)")
#     elif computer_choice == 2:
#         bot.send_message(message.chat.id, "Ты выиграл! (выход - exit)")
#     else:
#         bot.send_message(message.chat.id, "Ты проиграл! (выход - exit)")
...
def info(message):
  bot.send_message(
    message.chat.id,
    "Бот, который присылает анекдоты в 12:00 или при написании цифры\n"
...
def main(message):
  try:
    ...
    elif message.text[0:3].lower() == 'кнб':
    ...
    bot.send_message(message.chat.id, 'Error! Try again later!')
